...
from main_pretrain_videomae import get_model as get_videomae_model
from src.engine_eval import eval_vpt
from src.data.datasets import build_vpt_eval_loader
...
def get_model(args):
    ...
    elif 'videomae' in args.model:
        return get_videomae_model(args)
    else:
...
def get_args_parser():
    ...
    parser.add_argument("--task", required=True, choices=['perspective', 'depth', 'exp'])
    #DINO parameters
...
def main(args):
    ...
    elif 'autoreg' in args.model or 'videomae' in args.model:
        if args.use_cce:
    ...
    train_loader, val_loader, test_loader, human_loader = build_vpt_eval_loader(args)
    ...
    # else:
    #     log_writer = None
    outputs = eval_vpt(model, train_loader, val_loader, test_loader, human_loader, device, 0, args=args, timm_model=args.timm_model)
    print(outputs)
    return outputs
